...
class Joc:
    ...
    def mutari(self, jucator_opus):
        ...
        for j in range(self.NR_COLOANE):
            linie = 0
            # print(len(config))
            # print(linie * self.NR_COLOANE + j)
            # print('\n')
            while linie < self.NR_LINII -1:
                if config[(linie+1) * self.NR_COLOANE + j] == '.':
                    linie += 1
                else:
                    break
            if config[(linie) * self.NR_COLOANE + j] == '.':
                print ((linie,j))
                copie = [config[x] for x in range(len(config))]
                copie[(linie) * self.NR_COLOANE + j] = jucator_opus
                mutare = Joc(copie)
    ...
    def nr_intervale_deschise(self, jucator):
        ...
        # linii
        for i in range(self.NR_LINII):
            for j in range(self.NR_COLOANE - 4):
                for k in range(4):
                    if k == 0 and self.matr[i * self.NR_COLOANE + j + k] != jucator:
                        break
                    elif self.matr[i * self.NR_COLOANE + j + k] != jucator:
                        break
                    if k == 3:
                        rez += 1
        ...
        for i in range(self.NR_LINII - 4):
            for j in range(self.NR_COLOANE):
                for k in range(4):
                    if k == 0 and self.matr[(i + k) * self.NR_COLOANE + j] != jucator:
                        break
                    elif self.matr[(i + k) * self.NR_COLOANE + j] != jucator:
                        break
                    if k == 3:
                        rez += 1
        ...
        for i in range(self.NR_LINII - 1, 2, -1):
            for j in range(self.NR_COLOANE - 1, 2, -1):
                for k in range(4):
                    if k == 0 and self.matr[(i - k) * self.NR_COLOANE + j - k] != jucator:
                        break
                    elif self.matr[(i - k) * self.NR_COLOANE + j - k] != jucator:
                        break
                    if k == 3:
                        rez += 1
